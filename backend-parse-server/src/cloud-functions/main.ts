...
Parse.Cloud.define("sendVolunteerContractsByMail", async (request) => {
    return await Container.get(SendVolunteerContractsByMail).run(request);
}, {
    fields: ['eventId', 'userIds'],
    requireUser: true,
    requireAllUserRoles: [ROLE_EVENT_ORGANIZER]
});

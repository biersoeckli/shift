...
export interface VerifyAuthChallengeCodeResult {
    username: string;
    sessionKey: string;
}

@Service()
export class VerifyAuthChallengeCodeFunction extends BaseCloudFunction<VerifyAuthChallengeCodeResult> {
    ...
    async run(request: Parse.Cloud.FunctionRequest<Parse.Cloud.Params>) {
        ...
        return { username: user.get('username'), sessionKey: generatedPassword } as VerifyAuthChallengeCodeResult;
    }
}
